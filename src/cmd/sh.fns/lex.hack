...
static	char	lookt[STRSIZE];		/* lookahead token */
...
again:
	...
	if (!class(c, MNAME)) {
		...
		if (keyflag) {
			...
			/* Look for following parens indicating sh function. */
			if (c == _NAME) {
				bpp = savebuf();
				s = balloc(strlen(strt) + 1);
				strcpy(s, strt);
				lookahead = yylex();
				strcpy(lookt, strt);
				strcpy(strt, s);
				freebuf(bpp);
printf("_NAME found, lookahead=%d, strt=%s, lookt=%s\n", lookahead, strt, lookt);
				return (lookahead == _PARENS) ? _FNNAME : _NAME;
			}
		}
		return c;
	}
