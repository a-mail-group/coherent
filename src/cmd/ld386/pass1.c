...
int		loadsw;
{
	...
	if (size < sizeof (FILHDR)) {
		modmsg (fname, mname, "not an object file - length %d", size);
		/* This cannot be an object file */
		exit (1);
	}
	mp = alloc (sizeof (* mp));	/* allocate our header */
	mp->f = alloc ((size_t) size);	/* allocate space for file */
	endmod = (char *) mp->f + size;	/* end of space for file */
	xread (mp->f, (size_t) size);	/* inhale file */
	 ...
	 */
	j = (long) (mp->f);
	mp->s = (SCNHDR *) (sizeof (FILHDR) + j + mp->f->f_opthdr);
	mp->f->f_symptr += j;
	mp->l = (char *) (mp->f->f_symptr + mp->f->f_nsyms * sizeof (SYMENT));
	if (mp->f->f_symptr > (long) endmod || mp->l > endmod ||
	    mp->f->f_nsyms < 0)
		corrupt (mp);
...
#undef tst
		...
		if (k == osegs && loadsw) {	/* New segment */
			if (++ osegs == MAXSEG)
				fatal ("Max segment limit of %d exceeded",
				       MAXSEG);
...
#else
# error	I need to be programmed for your machine.
#endif
...
int		loadsw;
{
	...
	sscanf (in_arh.ar_size, "%ld", & ar_size);
	...
	i = size = ar_size - size - sizeof (count);
	if (i != size)
	...
	/* search symbol table until nothing found */
	do {
		for (found = i = 0, name = names;
			...
			object (in_arh.ar_name, ar_size, loadsw);
...
mod_t * mp;
{
	...
	sec = s->n_scnum;		/* section number in this module */
	switch (s->n_sclass) {
	case C_EXTDEF:
		s->n_sclass = C_EXT;
	...
	if (sec > 0) {
		/*
		 * The output section number ordering might not match the
		 * order of sections in this module, watch out.
		 * The output section number has been stashed in the s_paddr
		 * field, GOK...
		 */
		osec = mp->s[sec-1].s_paddr;
	}
	name = symName (s, w1);
	...
	if (sec > 0)
		s->n_value += secth[osec].s_size - mp->s[sec-1].s_vaddr;
				
	if (local == new &&
 	    (nolcl || (noilcl && name [0] == '.' && name [1] == 'L')))
		return;
