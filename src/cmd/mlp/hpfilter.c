...
main(argc, argv) int argc; char *argv[];
{
	...
	for (nn = 1; nn < argc; nn++) {
		if (!strncmp("-b", argv[nn], 2)) 
			bottomtray = 1;
		else if (!strncmp("-B", argv[nn], 2)) 
			bottomtray = 1;
		else if (!strncmp("-e", argv[nn], 2)) 
			envelope = 1;
		else if (!strncmp("-E", argv[nn], 2)) 
			envelope = 1;
	} 
 
	printf("\033&l%dH", (bottomtray == 1) ? 4 : 1);

	if (envelope == 1) {
		printf(
			"\033&l81A"	/* Select Envelopes */
			"\033&l1O"	/* Landscape Mode */
			);
		lines = 21;
		DECI  = 2510;
	} else {
		printf(
			"\033&12A"	/* Select Normal Paper */
			"\033&l0O"	/* Portrait Mode */
			);
		lines = 66;
		DECI  = 7416;
	}
	...
	printf(
		"\033&a0h%dV"		/* Home HP cursor */
		"\033(11U"		/* PC-8 font (11U is Danish) */
		"\033(s0B",		/* Lightweight stroke */
		m);
	...
	printf(
		...
		"\033&l0O"		/* Portrait Mode */
		"\033&12A"		/* Select Normal Paper */
		"\033*c1F"		/* Delete ALL temporary soft fonts */
		);
	exit(EXIT_SUCCESS);
}

/* end of mlp/hpfilter.c */
