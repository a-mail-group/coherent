...
int		count;
{
	...
	g0->control = 0;
	doscsi(r, 4, pritape, ctsleepPri(c, slpriSigCatch), "writefm");
...
int		wait;
{
	...
	doscsi(r, 2, pritape, ctsleepPri(c, slpriSigCatch), "rewind");
...
register ctctrl_p	c;
{
	...
	g0->xfr_len = ((unsigned char *) &xfrsize)[0];
	g0->control = 0;
	doscsi(r, 1, pritape, ctsleepPri(c, slpriSigCatch), "ctblkwrt");
	switch (r->status) {
	case ST_GOOD:
		c->start = c->cache;
		c->avail = HAICACHESZ;
