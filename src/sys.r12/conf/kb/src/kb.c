...
#define	KB_CTRL_KEY	(0x1D - 1)	/* Control key */

#define	KB_ALT_KEY	(0x38 - 1)	/* Alt key or ALT GR */
...
/*
 * Default function key strings (terminated by -1 [\xFF])
 */
...
static char * deffuncs [] = {
	...
	/* Shifted function keys - no f11, f12 on 83-key keyboard */
...
unsigned char fr_agmaptab [] ={					/* Alt Gr */
	       XXX,  XXX,  '~',  '#',  '{',  '[',  '|',		/* 1 - 7 */
	 '`', '\\',  '^',  '@',  ']',  '}',  XXX,  XXX,		/* 8 - 15 */
...
static	char	scroll_lock;		/* Scroll lock state */
static  char	lshift_state = LSHIFT;	/* Left shift alternate state */
...
#define KDDEBUG 0
	...
	case TIOCSHIFT:   /* switch left-SHIFT and "\" */
		lshift_state = LSHIFTA;    /* alternate values */
		lmaptab [41] = '\\';
		lmaptab [42] = XXX;
		umaptab [41] = '|';
		umaptab [42] = XXX;
	...
	case TIOCCSHIFT:  /* normal (default) left-SHIFT and "\" */
		lshift_state = LSHIFT;     /* normal values */
		lmaptab [41] = XXX;
		lmaptab [42] = '\\';
		umaptab [41] = XXX;
		umaptab [42] = '|';
 ...
 *
 *	Notes:	isbatch () was scheduled as a deferred process by kbintr ().
 */
...
kbintr ()
{
	...
	if (s & SHFT) {
		if (r & KEYUP) {			/* "shift" released */
			if (c == RSHIFT)
				shift &= ~ SRS;
			else if (c == lshift_state)
				shift &= ~ SLS;
			else if (c == KB_CTRL_KEY)
				shift &= ~ CTS;
			else if (c == KB_ALT_KEY) {
				if (kb_lang == kb_lang_fr || kb_lang == kb_lang_de)
	 ...
	 */
	if ((shift & AGS) && (kb_lang == kb_lang_fr || kb_lang == kb_lang_de))			/* Alt Graphics ? */
		c = agmaptab [c];
	else if (shift & CTS) {
...
int c;
{
	...
	case 64: case 65: case 66: case 67: case 68:
		...
		else if ((shift & SES) != 0 && (shift & CTS) != 0) /* ctl-shift-Fx */
			cp = isfval [c - 29];
		else if ((shift & CTS) != 0)		/* ctl-Fx */
			cp = isfval [c - 39];
		else if ((shift & SES) != 0)		/* shift-Fx */
			cp = isfval [c - 49];
		else
			cp = isfval [c - 59];		/* Fx */
		break;
